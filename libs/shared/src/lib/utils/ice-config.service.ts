...
export class IceConfigService {
    ...
    async getConfig(): Promise<RTCConfiguration> {
        ...
        try {
            ...
            console.log(
                'ICE configuration loaded successfully:',
            ...
            console.warn(
                'Failed to fetch ICE config from server, using STUN-only fallback:',
                errorMessage
            );
            console.warn(
                'This is expected if the backend is unreachable. You can still use the application with STUN servers only.'
            );
